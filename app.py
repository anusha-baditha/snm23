...
import re
mydb = connection.MySQLConnection(user='flaskuser', password='password',
                              host='localhost',
                              database='flaskdb')
app=Flask(__name__)
    ...
    if session.get('user'):
        try:
            ...
            if user:
                ...
                flash('could not verify user')
                return rtedirect(url_for('dashboard'))
        except Exception as e:
...
def deletefile(fid):
    if session.get('user'):
        try:
            ...
            return rtedirect(url_for('allfilesdata'))
        else:
        ...
        if pattern.match(sdata):
            try:
                ...
                if user:
                    ...
                    searchnotesdata=cursor.fetchall() 
...
def newpassword(data):
    if request.method=='PUT':
        try:
            ...
            return redirect(url_for('newppassword',data=data))
        else:
